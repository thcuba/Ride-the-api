...
class CaptureSession(BaseModel):
    ...
    session_id: str
    type: str = "capture"  # boot, command, polling, handshake
...
# ── Client section (what the device sends) ────────────────────────────────────

class AuthConfig(BaseModel):
    """Authentication info for the device."""
    type: str = "bearer"  # bearer, basic, custom
    token_endpoint: str = ""
...
# ── Server section (what the proxy responds) ──────────────────────────────────
...
class FieldMapping(BaseModel):
    ...
    source: str  # 'request.body.x.y', 'request.headers.X', 'constant.z'
